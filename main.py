...
def admin_only(f):
    ...
    def decorated_function(*args, **kwargs):
        if current_user.is_anonymous == True or current_user.id != 1:
            return abort(403, description="You don't have rights to access this page")
...
def register():
    ...
    if request.method == "POST":
        if register_form.validate_on_submit() == True:
            ...
            if user:
                flash(message="You've already registered with this email")
...
def show_post(post_id):
    ...
    if commnet_form.validate_on_submit() == True:
        if not current_user.is_authenticated:
            flash("You need to login or register to comment.")
            return redirect(url_for("login"))

        new_comment = Comment(
            text=commnet_form.comment_text.data,
    ...
    return render_template("post.html", post=requested_post, form=commnet_form, current_user=current_user)
